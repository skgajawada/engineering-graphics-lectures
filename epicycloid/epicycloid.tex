...
\scriptsize \insertframenumber\hspace{1.2em}
\end{beamercolorbox}}
...
\newcommand{\GenPt}[1]{\coordinate(G#1)at($(Czero)+({330+30*(#1)}:15)$);\fill[black](G#1)circle(2.4pt);}
...
\newcommand{\CenterPt}[1]{\coordinate(C#1)at({150-10*(#1)}:60);\fill[black](C#1)circle(2.4pt);\node[font=\tiny,above right,inner sep=.5pt]at(C#1){$C_{#1}$};}
...
\begin{align*}
\text{Diameter of directing circle} &= 90\text{ mm},\qquad R=45\text{ mm},\\
\text{Diameter of generating circle} &= 30\text{ mm},\qquad r=15\text{ mm},\\
\angle AOB &=120^\circ,\qquad \text{Number of divisions}=12.
\end{align*}
\end{frame}
...
\text{Generating-circle division angle} &=\frac{360^\circ}{12}=30^\circ.
\end{align*}
\end{frame}
...
\begin{frame}{Step 18: Mark $M$ on the directing circle}\centering\begin{tikzpicture}[scale=.075,>=stealth]\FinalConstruction\SelectedGeometry\PGeometry\PtoCpArc\draw[contact line](O)--(Cp);\fill[black](M)circle(2.4pt);\node[font=\scriptsize,above right]at(M){$M$};\Legend{$M=OC_p\cap AB$.}\end{tikzpicture}\end{frame}
\begin{frame}{Step 19: Draw the normal $NN'$ through $P$}\centering\begin{tikzpicture}[scale=.075,>=stealth]\FinalConstruction\SelectedGeometry\PtoCpArc\draw[contact line](O)--(Cp);\NormalAndTangent\PGeometry\fill[black](M)circle(1.8pt)node[above right]{$M$};\fill[black](Cp)circle(1.8pt)node[above right]{$C_p$};\Legend{$NN'$: normal.}\end{tikzpicture}\end{frame}
\begin{frame}{Step 20: Draw the tangent perpendicular to $NN'$}\centering\begin{tikzpicture}[scale=.075,>=stealth]\FinalConstruction\SelectedGeometry\PtoCpArc\draw[contact line](O)--(Cp);\NormalAndTangent\PGeometry\fill[black](M)circle(1.8pt)node[above right]{$M$};\fill[black](Cp)circle(1.8pt)node[above right]{$C_p$};\Legend{$TT'\perp NN'$.}\end{tikzpicture}\end{frame}
\begin{frame}{Step 20: Show the right-angle at $P$}\centering\begin{tikzpicture}[scale=.075,>=stealth]\FinalConstruction\SelectedGeometry\PtoCpArc\draw[contact line](O)--(Cp);\NormalAndTangent\PGeometry\fill[black](M)circle(1.8pt)node[above right]{$M$};\fill[black](Cp)circle(1.8pt)node[above right]{$C_p$};\pic[draw=black,line width=1pt,angle radius=7pt,angle eccentricity=1.35]at(Psel){right angle=T2--Psel--NNright};\Legend{Right angle at $P$.}\end{tikzpicture}\end{frame}
\begin{frame}{Step 20: Final completed epicycloid construction}\centering\begin{tikzpicture}[scale=.075,>=stealth]\FinalConstruction\SelectedGeometry\PtoCpArc\draw[contact line](O)--(Cp);\NormalAndTangent\PGeometry\fill[black](M)circle(1.8pt)node[above right]{$M$};\fill[black](Cp)circle(1.8pt)node[above right]{$C_p$};\pic[draw=black,line width=1pt,angle radius=7pt,angle eccentricity=1.35]at(Psel){right angle=T2--Psel--NNright};\Legend{Completed epicycloid with tangent and normal.}\end{tikzpicture}\end{frame}
\end{document}
