...
\newcommand{\DrawCzero}{%
    ...
    \fill[black] (Czero) circle (2.0pt) node[above left,font=\scriptsize,black,xshift=-2pt,yshift=2pt] {$C_0$};
}
...
\newcommand{\CenterPt}[1]{%
    ...
    \fill[black] (C#1) circle (2.0pt);
    \node[font=\scriptsize,black,above right,xshift=3pt,yshift=3pt,inner sep=1pt,fill=white]
        at(C#1) {$C_{#1}$};
}
...
\newcommand{\SelectedGeometry}{%
    ...
    \coordinate(NNleft)  at ($(Psel)+(\NormAng+180:48)$);
...
\begin{tikzpicture}[scale=.075,>=stealth,rotate=-90]
    ...
    \fill[black] (M) circle(2.0pt);
    \node[font=\scriptsize,black,above right,xshift=8pt,yshift=8pt,fill=white] at(M) {$M$};
