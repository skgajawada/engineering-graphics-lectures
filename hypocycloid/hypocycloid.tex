...
\newcommand{\DrawCzero}{%
    ...
    \fill[black] (Czero) circle (1.0pt) node[above left,font=\scriptsize] {$C_0$};
}
...
\newcommand{\CenterPt}[1]{%
    ...
    \fill[black] (C#1) circle (1.0pt);
    \node[font=\scriptsize,above right,inner sep=.8pt]
        at(C#1) {$C_{#1}$};
}
...
\begin{tikzpicture}[scale=.075,>=stealth,rotate=-90]
    ...
    \fill[black] (M) circle(1.0pt);
    \node[font=\scriptsize,above right,xshift=4pt,yshift=4pt] at(M) {$M$};
