...
\newcommand{\SelectedGeometry}{%
    % First OP=50 mm intersection on the one-revolution hypocycloid.
    % theta_P = 36.79400947 deg.
    \coordinate(Psel) at (-26.75658139,42.23843454);
    \coordinate(Cp) at (98.20599053:60);
    \coordinate(M)  at (98.20599053:85);

    % PM is the normal.
    \def\NormAng{70.75580663}
    \coordinate(NNright) at ($(Psel)+(\NormAng:35)$);
    \coordinate(NNleft)  at ($(Psel)+(\NormAng+180:35)$);

    % Tangent perpendicular to PM.
    \coordinate(T1) at ($(Psel)+(160.75580663:35)$);
    \coordinate(T2) at ($(Psel)+(340.75580663:35)$);
}
...
\newcommand{\PtoCpArc}{%
    % PC_p = 25 mm; direction P -> C_p = 43.30562273 deg.
    \draw[construction line,very thin]
        (Psel)+(31.30562273:25)
        arc(31.30562273:55.30562273:25);
    ...
    % Dimension line parallel to PC_p; limiting lines perpendicular.
    \draw[dimension line]
        ($(Psel)+(133.30562273:6)$)--($(Cp)+(133.30562273:6)$)
        node[
            midway,
            sloped,
            above,
    ...
    \draw[thin,black!55]
        (Psel)--($(Psel)+(133.30562273:6)$);
    \draw[thin,black!55]
        (Cp)--($(Cp)+(133.30562273:6)$);
}
...
\begin{frame}{Step 15: Locate $P$ such that $OP=50$ mm}
\centering
\begin{tikzpicture}[scale=.075,>=stealth]
    \FinalConstruction
    \SelectedGeometry

    % 50 mm locus centred at O.
    \draw[construction line,very thin] (O) circle (50);

    \draw[contact line] (O)--(Psel);

    % Dimension line parallel to OP.
    \draw[dimension line]
        ($(O)+(212.35283853:7)$)--($(Psel)+(212.35283853:7)$)
        node[
            midway,
            sloped,
            below,
    ...
    \draw[thin,black!55]
        (O)--($(O)+(212.35283853:7)$);
    \draw[thin,black!55]
        (Psel)--($(Psel)+(212.35283853:7)$);

    \PGeometry
    \Legend{The $50$ mm circle intersects the hypocycloid at the required point $P$.}
\end{tikzpicture}
\end{frame}
